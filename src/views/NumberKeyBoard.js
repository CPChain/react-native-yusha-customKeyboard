...
class Keyboard extends PureComponent {
  ...
  _renderNumberKeys () {
    const numberKeysShuffle = []
    const randomKey = numberKeysArr
    // console.log(randomKey)
    numberKeysShuffle.push(randomKey.slice(0, 3))
    numberKeysShuffle.push(randomKey.slice(3, 6))
    numberKeysShuffle.push(randomKey.slice(6, 9))
    const left = randomKey[randomKey.length - 1].mainText
    this.setState({ left })
    // console.log(numberKeysShuffle)
    return numberKeysShuffle.map((group, groupIndex) => {
      return (
        ...
        <View style={styles.main}>
          ...
          <View style={styles.row}>
            ...
            <TouchableHighlight
              underlayColor={BG_COLOR}
              style={keyStyle.wrapper}
              onPress={this._onPress.bind(this, this.state.left)}
            >
              <View style={[keyStyle.bd, this._disableBorder()]}>
                <Text style={keyStyle.mainText}>{this.state.left}</Text>
              </View>
            </TouchableHighlight>
